...
const DashEntry = ({body}) => {
    ...
    if (body.color === 'red' && body.exp_name === 'Settle Up Amount'){
        exp = `You got ${localStorage.getItem("currency")} ${body.expense.toFixed(2)} to ${body.lender} for ${body.exp_name}`
    }
    else if (body.color === 'green' && body.exp_name === 'Settle Up Amount'){
        exp = `You have paid ${localStorage.getItem("currency")} ${body.expense.toFixed(2)} to ${body.borrower} for ${body.exp_name}`
    }
    else if (body.color === 'red' && body.exp_name !== 'Settle Up Amount'){
        exp = `You pay ${localStorage.getItem("currency")} ${body.expense.toFixed(2)} to ${body.lender} for ${body.exp_name}`
    }
    else{
        exp = `You get ${localStorage.getItem("currency")} ${body.expense.toFixed(2)} to ${body.borrower} for ${body.exp_name}`
    }
    return (
