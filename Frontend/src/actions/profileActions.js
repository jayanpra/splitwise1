...
export const get_data = createAsyncThunk(
    'users/fetchProfile',
    async (token) => {
      const response = await axios.post('http://18.237.56.160:3001/profile/initialPull', {'token': token})
...
export const send_update = createAsyncThunk(
  'users/sendupdate',
  async (pckg) => {
    const response = await axios.post('http://18.237.56.160:3001/profile/update', pckg)
...
export const sendProfileImage = createAsyncThunk(
  ...
  async (formData) => {
    ...
    const response = await axios.post('http://18.237.56.160:3001/imageupdate',formData,config )
     if (response.status === 200) {
