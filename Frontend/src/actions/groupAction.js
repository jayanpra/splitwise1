...
export const get_group = createAsyncThunk(
    'users/fetchGroup',
    async (pckg) => {
      const response = await axios.post('http://54.190.4.247:3001/groupFill', pckg)
...
export const change_group = createAsyncThunk(
    'users/changeGroup',
    async (pckg) => {
      const response = await axios.post('http://54.190.4.247:3001/groupChange', pckg)
...
export const approve_group = createAsyncThunk(
  'users/approveGroup',
  async (pckg) => {
    const response = await axios.post('http://54.190.4.247:3001/altergroup', pckg)
    console.log(response.data)
    if (response.status === 200){
...
export const exit_group = createAsyncThunk(
  ...
  async (pckg, group_name) => {
    const response = await axios.post('http://54.190.4.247:3001/groupExit', pckg)
    console.log(response.data)
    if (response.status === 200){
